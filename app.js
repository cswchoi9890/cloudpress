...
const CP = {
  ...
  async api(path, opts = {}) {
    ...
    try { data = await res.json(); } catch { data = { ok: false, error: '서버 응답을 처리할 수 없습니다.' }; }
    if (res.status === 401) { this.clearAuth(); window.location.href = '/auth.html'; return data; }
    if (res.status === 404) return { ok: false, error: data?.error || 'API 엔드포인트를 찾을 수 없습니다.' };
    if (res.status >= 500) return { ok: false, error: data?.error || `서버 오류가 발생했습니다. (${res.status})` };
    return data;
  },
